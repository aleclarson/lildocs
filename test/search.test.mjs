...
test("uses theme colors for search input placeholder and focus styles", async () => {
  ...
  assert.match(css, /\.toc \{[^}]*overflow-x: hidden/);
  assert.match(css, /\.tocLinks a \{[^}]*overflow-wrap: anywhere/);
  assert.match(css, /\.tocLinks a \{[^}]*white-space: normal/);
  assert.doesNotMatch(css, /\.tocLinks a \{[^}]*text-overflow: ellipsis/);
...
test("emits collapsible sidebar controls with Tabler icons", async () => {
  ...
  assert.match(html, /sidebarCollapseIcon ti ti-layout-sidebar-left-collapse/);
  assert.match(html, /sidebarMenuIcon ti ti-menu-2/);
